...
namespace FlowFields.Data
{
    public static class MapDataBlobFactory
    {
        public static BlobAssetReference<MapDataBlob> Generate(int width)
        {
            ...
            AllocateLifeState(ref blobBuilder, ref mapDataBlob);
            var blobAssetReference = blobBuilder.CreateBlobAssetReference<MapDataBlob>(Allocator.Persistent);
            blobBuilder.Dispose();
            return blobAssetReference;
        }
        private static void AllocateLifeState(ref BlobBuilder blobBuilder, ref MapDataBlob mapDataBlob)
        {
            var states = blobBuilder.Allocate(ref mapDataBlob.LifeState, 512);
            ...
            for (int i = 0; i < 512; i++)
            {
                ...
                var isAlive = (i & 1 << 8) != 0;
                states[i] = (byte)((isAlive && neighboursCount == 2) || neighboursCount == 3? 1 : 0);
            }
        }
        private static void AllocateCodeIndexes(ref BlobBuilder blobBuilder, ref MapDataBlob mapDataBlob, int width)
        {
            ...
            for (var code = 0; code < 256; code++)
            {
                ...
                for (byte directionIndex = 0; directionIndex < 8; directionIndex++)
                {
                    ...
                    directionData.IndexOffset = (short) moveDirectionIndexOffset[directionIndex];
    ...
    public struct MapDataBlob
    {
        ...
        public BlobArray<byte> LifeState;
